...
<form id="moderate" name="moderate" method="post" action="topicadmin.action?action=moderate&operation=${operation}">
		<input type="hidden" name="topicid" value="${topiclist}" />
		<input type="hidden" name="forumid" value="${forumid}" />
		<input type="hidden" id="referer" name="referer" value="showforum.action?forumid=${forumid}">
		...
		<tbody>
			<tr>
				...
				<td>
					...
					<SPAN class="smalltxt">即将与这个主题合并的主题id,如showtopic.action?topicid=22，tid 为 22</SPAN>
				</td>
			</tr>
		</tbody>
		</#if>
		...
		<tbody>
			<tr>
				...
				<td>
					...
					<input name="postid" type="checkbox" value="${post.pid}" /><strong>${post.poster}</strong><br />
						${post.message}<br />
					</#list>
				</td>
			</tr>
		...
		<tbody>
			<tr>
				...
				<div style="position: relative;">
					...
					<#list postlist as post>
					<div name="answeroption" <#if post.users.uid!=topicinfo.posterid>onclick="setValuableOrBestAnswer(this, ${post.pid});" style="cursor: pointer; width: 100%;"</#if>>
					<strong>{post[poster]}</strong>&nbsp; <#if post.users.uid!=topicinfo.posterid>得分: <input name="postbonus" id="bonus_${post.pid}" type="text" value="0" size="3" maxlength="9" onblur="checkInt(this);" onmouseover="bonushint(this);" onmouseout="closebonushint(this);" /><input name="addons" type="hidden" value="${post.users.uid}|${post.pid}|${post.poster}" /><#else>不能给自己分</#if><br />
						${post.message}<br />
					</div><br />
					</#list>
