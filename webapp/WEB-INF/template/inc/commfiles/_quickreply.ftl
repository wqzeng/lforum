...
<div id="quickpost" class="box">
	...
	<div class="postform">
		...
		<#if isenddebate==false>
			...
			<input type="text" id="title" name="title" size="84" tabindex="1" value="" /><input type="hidden" id="postid" name="postid" value="-1" />
		</h5>
		<p><label>内容</label>
		<textarea rows="7" cols="80" name="message" id="message" onKeyDown="ctlent(event,false);" tabindex="2" class="autosave" style="background:url(${quickbgadimg}) no-repeat 0 0; " <#if quickbgadlink!=""> onfocus="$('adlinkbtn').style.display='';$('closebtn').style.display='';this.onfocus=null;"</#if>></textarea>
		</p>
		<#if isseccode>
		<p>验证码:<#include "_vcode.ftl"/></p>
		</#if>
		<p class="btns">
			...
			<input  name="topicsreset" id="restoredata" value="清空内容" tabindex="6" title="清空内容" onclick="javascript:document.postform.reset(); return true; " type="reset" />
			<input name="restoredata" id="restoredata" value="恢复数据" tabindex="5" title="恢复上次自动保存的数据" onclick="loadData();" type="button" />
