...
<#else>
<form id="form1" name="form1" method="post" <#if loginauth!="">action="login.action?loginauth=${loginauth}&referer=${referer}"<#else>action=""</#if>>
<div class="mainbox formbox">
	...
	<tr>
		...
		<td>
			<input name="username" type="text" id="username" size="20" value="${postusername}" tabindex="4" />  &nbsp; <a href="register.action" tabindex="-1" accesskey="r" title="立即注册 (ALT + R)">立即注册</a>
		</td>
	</tr>
	</tbody>
<#if loginauth=="">
	...
	<tr>
		...
		<td> 
		<input name="password" type="password" id="password" size="20" tabindex="5" /> &nbsp; <a href="getpassword.action" tabindex="-1" accesskey="g" title="忘记密码 (ALT + G)">忘记密码</a>
		</td>
	</tr>
	</tbody>
</#if>	
	...
	<tbody>
		...
		<td>
			...
			<input name="cancel" type="button" id="cancel" value="取消" onclick="javascript:window.location.replace('./main.action')"/>
		</td>
		</tr>
	</tbody>
	</table>
