...
function diggdebate_callback(doc) {
	...
	var err = doc.getElementsByTagName('error');
		if (err[0] != null && err[0] != undefined)
		{
			...
			$('diggs'+cureentpid).innerHTML = parseInt($('diggs'+cureentpid).innerHTML)+1;
			$('cliktalk'+cureentpid).style.display='none';
			return ;
		}
}
...
function showDebatReplyBox(tid, pid, opinion, parseurloff, smileyoff, bbcodeoff)
{
	...
	html += '		<tr>';
	html += '			<td>';
	html += '		<textarea name="message" cols="10" rows="7" class="autosave"  tabindex="2" onkeydown="debatequickreply(event, this.form);"  onfocus='; 
	...
	{
		html +='<div id="js_vcode" name="js_vcode"></div>';
	...
	if($('debate_vcode')!=undefined)
	{
	$('js_vcode').innerHTML=$('debate_vcode').innerHTML;
	}
}
...
function ctlent(event, theform) {
	if(postSubmited == false && (event.ctrlKey && event.keyCode == 13) || (event.altKey && event.keyCode == 83) && $('postsubmit')) {
	    try {
		    if(in_array($('postsubmit').name, ['topicsubmit', 'replysubmit', 'editsubmit', 'pmsubmit']) && !validate(theform)) {
			    doane(event);
			    return;
		    }
		    postSubmited = true;
