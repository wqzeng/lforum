...
function showrate(pid,aspxrewrite)
{
	...
	if (ratediv.innerHTML == "")
	{
		...
		var action = "tools/ajax.action?t=ratelist";
...
function outputRatelog(doc, pid, aspxrewrite)
{
	...
	for (var i = 0; i < ratelist.length; i++ )
	{
        ...
        if(aspxrewrite == 1)
        {
		    html += "<tr><td><a target='_blank' href='userinfo-" + uid + ".action'>" + username + "</a></td><td>&nbsp;&nbsp;" + postdatetime + "</td><td>&nbsp;&nbsp;" + extcreditsname + "&nbsp;&nbsp;<b>" + score + "</b>&nbsp;" + extcreditsunit + "</td><td>&nbsp;&nbsp;" + reason + "</td></tr>";
		}
		else
		{
		    html += "<tr><td><a target='_blank' href='userinfo.action?userid=" + uid + "'>" + username + "</a></td><td>&nbsp;&nbsp;" + postdatetime + "</td><td>&nbsp;&nbsp;" + extcreditsname + "&nbsp;&nbsp;<b>" + score + "</b>&nbsp;" + extcreditsunit + "</td><td>&nbsp;&nbsp;" + reason + "</td></tr>";
		}
	}
	html += "</table>";
...
{
	_sendRequest('tools/ajax.action?t=gettopictags&topicid=' + topicid, function(responseText){
		if (responseText)
		{
			...
			{			
				...
				for (var i in topic_tags)
				{
					...
					if (aspxrewrite == 1)
					{
						html += 'topictag-' + topic_tags[i].tagid + '.action"';
					}
					else
					{
						html += 'tags.action?t=topic&tagid=' + topic_tags[i].tagid +'"';
					}
...
function parsetag() {
	...
	$('firstpost').innerHTML = str.replace(/<h_ (\d+)>/ig, function($1, $2) {
		var temp_html = '<span href=\"tools/ajax.action?t=topicswithsametag&tagid=' + tagfindarray[$2].tagid + '\" onclick=\"tagshow(event)\" class=\"t_tag\"';
