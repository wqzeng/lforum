...
function ajaxctlent(event, objfrm, topicid, isendpage, templatepath) {
	if(postSubmited == false && (event.ctrlKey && event.keyCode == 13) || (event.altKey && event.keyCode == 83)) {
		if (!$("postsubmit").disabled)
		{
			...
			{
				sendRequest('tools/ajax.action?topicid=' + topicid + '&t=quickreply', isendpage, templatepath);			
			}
		}
		else
		{
...
function saveResult(doc, templatepath)
{
	...
	html += '					<a href="showtree.action?topicid=' + topicid + '&postid=' + pid + '">树型</a><span class="see-line2">|</span>';
	html += '					<a href="favorites.action?topicid=' + topicid + '">收藏</a><span class="see-line2">|</span>';
	html += '					<a href="editpost.action?topicid=' + topicid + '&postid=' + pid + '&referer=' + escape(window.location) + '">编辑</a><span class="see-line2">|</span>';
	html += '					<a href="delpost.action?topicid=' + topicid + '&postid=' + pid + '" onclick="return confirm(\'确定要删除吗?\');">删除</a>&nbsp;';
	if(ismoder == 1)
	{
	...
	html += '					<a href="postreply.action?topicid='+topicid+'&postid='+pid+'&quote=yes">引用</a>|';
	...
	html += '							<a href="usercppostpm.action?msgtoid=' + posterid + '" target="_blank">发送短消息</a></p>';
	//html += '						</div>';
	//html += '						<div class="popupmenuitem">';
	html += '							<p><a href="userinfo.action?userid=' + posterid + '" target="_blank">查看公共资料</a></p>';
	html += '							<p><a href="search.action?posterid=' + posterid + '">查找该会员全部帖子</a></p>';
	//html += '						</div>';
	//html += '						<div class="popupmenuitem">';
	html += '						<ul>';
	...
	if (digestposts>0)
	{
		html += '						<a href="search.action?posterid=' + posterid + '&type=digest">' + digestposts + '</a>'
	}
	else
	{
		digestposts
