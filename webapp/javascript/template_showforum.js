	...
	var dataArrayLen = dataArray.length;
		...
		for(var i = 0; i < dataArrayLen; i++){
			...
			for (j=0;j<10;j++){
				...
				{
				    list += "<li><a href=\"showtopic.action"
				    list += "?topicid=";
				    list += topicid;
				    if (ppp>0){
					    list += "&page=";
				...
				{
					list += "<a target=\"_blank\" href=\"userinfo.action?userid=";
...
function showtree(topicid,ppp,aspxrewrite){
	...
	{	
		...
		if(aspxrewrite == 1)
		{
		    ajaxRead("tools/ajax.action?t=topictree&topicid=" + topicid, "writetree(obj," + topicid + "," + ppp + "," + aspxrewrite + ");");
		}
		else
		{
		    ajaxRead("tools/ajax.action?t=topictree&topicid=" + topicid, "writetree(obj," + topicid + "," + ppp + ");");
		}
	}
