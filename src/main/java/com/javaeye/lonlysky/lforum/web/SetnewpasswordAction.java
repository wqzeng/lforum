...
public class SetnewpasswordAction extends ForumBaseAction {
	...
	public String execute() throws Exception {
		...
		authstr = !LForumRequest.getParamValue("id").equals("") ? LForumRequest.getParamValue("id")
				: LForumRequest.getParamValue("authstr");
		...
		if ((!userinfo.getUserfields().getAuthstr().equals(authstr))
				|| Utils.parseDate(userinfo.getUserfields().getAuthtime()).before(authtime)) {
		...
		if (LForumRequest.isPost()) {
			...
			if (Utils.isSafeSqlString(LForumRequest.getParamValue("newpassword"))
					&& userManager.updateUserPassword(uid, LForumRequest.getParamValue("newpassword"))) {
				userManager.updateAuthStr(uid, "", 0); //将验证码清空,并设置验证标志为无效

				reqcfg.setUrl("login.action").setMetaRefresh(5).setShowBackLink(false)
