...
public class UsercpinboxAction extends ForumBaseAction {
	...
	public String execute() throws Exception {
		...
		if (LForumRequest.isPost()) {
			...
			reqcfg.setUrl("usercpinbox.action").setMetaRefresh().setShowBackLink(true).addMsgLine("删除完毕");
		} else {
			bindItems();
		}
		return SUCCESS;
	...
	private void bindItems() {
		...
		pmlist = messageManager.getPrivateMessageList(userid, 0, 16, pageid, 2);
		pagenumbers = ForumUtils.getPageNumbers(pageid, pagecount, "usercpinbox.action", 8);
	}
