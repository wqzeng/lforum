...
public class SearchAction extends ForumBaseAction {
	...
	public String execute() throws Exception {
		...
		if (LForumRequest.isPost() || !LForumRequest.getParamValue("posterid").equals("")) {
			...
			if (useradminid != 1 && LForumRequest.getParamIntValue("keywordtype", 0) == 1
			...
			if (LForumRequest.getParamValue("keyword").equals("") && LForumRequest.getParamValue("poster").equals("")
					&& LForumRequest.getParamValue("posterid").equals("")) {
			...
			searchid = searchManager.search(userid, usergroupid, LForumRequest.getParamValue("keyword").trim(),
					posterid, type, searchforumid, keywordtype, searchtime, LForumRequest.getParamIntValue(
							"searchtimetype", 0), LForumRequest.getParamIntValue("resultorder", 0), LForumRequest
							.getParamIntValue("resultordertype", 0));
			...
			if (searchid > 0) {
				...
				if (type.equals("author")) {
					...
					topicpagenumbers = ForumUtils.getPageNumbers(topicpageid, topicpagecount,
							"search.action?show=topic&type=" + type + "&searchid=" + searchid, 8, "topicpage", "#1");
					return SUCCESS;
				} else {
				...
				//得到页码链接
				pagenumbers = ForumUtils.getPageNumbers(pageid, pagecount, "search.action?type=" + type + "&searchid="
						+ searchid, 8);
