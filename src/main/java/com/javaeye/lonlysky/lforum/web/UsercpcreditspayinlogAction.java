...
public class UsercpcreditspayinlogAction extends ForumBaseAction {
	...
	public String execute() throws Exception {
		...
		//获取主题总数
		payinlogcount = paymentLogManager.getPaymentLogInRecordCount(userid);
		//获取总页数
		...
		//获取收入记录并分页显示
		payloglist = paymentLogManager.getPayLogInList(pagesize, pageid, userid);

		pagenumbers = ForumUtils.getPageNumbers(pageid, pagecount, "usercpcreditspayinlog.action", 8);
		return SUCCESS;
	}
	...
	public int getPayinlogcount() {
		return payinlogcount;
	}
